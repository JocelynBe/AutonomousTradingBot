...
class RayProgressBar(ProgressBar):
...
def example():
    ray.init()
    num_ticks = 6
    pb = RayProgressBar(range(6 * 10))
    # You can replace this with any arbitrary Ray task/actor.
    tasks_pre_launch = [
        sleep_then_increment.remote(i, pb.progress_actor) for i in range(0, num_ticks)
    ]

    pb.print_until_done()
    tasks = ray.get(tasks_pre_launch)

    print(tasks, list(range(num_ticks)))
    print(num_ticks, ray.get(pb.progress_actor.get_counter.remote()))
